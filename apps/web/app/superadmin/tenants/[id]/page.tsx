...
import { TenantStatusActions } from '@/components/superadmin/tenant-status-actions'
...
export default async function TenantDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const activeSub = tenant.subscriptions.find((s) => ['ACTIVE', 'TRIALING'].includes(s.status))
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-3">
        <Link href="/superadmin/tenants" className="p-1.5 hover:bg-slate-800 rounded-lg text-slate-400">
          <ArrowLeft size={18} />
        </Link>
        <div>
      ...
      <div className="grid grid-cols-2 gap-5">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
          ...
          <Row
            label="Address"
            value={
              [tenant.address, tenant.city, tenant.state, tenant.pincode].filter(Boolean).join(', ') || '—'
            }
          />
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
          ...
          {activeSub ? (
            <>
              ...
              <Row
                label="Cycle"
                value={`${formatDate(activeSub.billingCycleStart)} → ${formatDate(activeSub.billingCycleEnd)}`}
              />
