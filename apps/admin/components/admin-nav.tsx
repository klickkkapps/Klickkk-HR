  ...
  return (
    <nav className="h-14 border-b border-slate-800 flex items-center px-6 gap-6">
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-sm text-slate-100">HRMS Super Admin</span>
      </div>

      <div className="flex gap-4 text-sm flex-1">
        <Link
          href="/"
          className={`${pathname === '/' ? 'text-slate-100' : 'text-slate-400 hover:text-slate-100'}`}
        >
          Dashboard
        </Link>
        <Link
          href="/tenants"
          className={`${pathname.startsWith('/tenants') ? 'text-slate-100' : 'text-slate-400 hover:text-slate-100'}`}
        >
          Tenants
        </Link>
      </div>
